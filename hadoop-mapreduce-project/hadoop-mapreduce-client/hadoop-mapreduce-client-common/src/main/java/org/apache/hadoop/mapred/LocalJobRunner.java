...
public class LocalJobRunner implements ClientProtocol {
  ...
  private class Job extends Thread implements TaskUmbilicalProtocol {
    ...
    protected class ReduceTaskRunnable extends RunnableWithThrowable {
      ...
      public void run() {
        try {
          ...
          reduce.setLocalMapFiles(mapOutputFiles); if (!Job.this.isInterrupted()) { reduce.setJobFile(localJobFile.toString());
    ...
    @Override
    public void run() {
      JobID jobId = profile.getJobID(); JobContext jContext = new JobContextImpl(job, jobId);
